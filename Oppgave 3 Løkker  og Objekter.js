...
for (let i = 0; i < people.length; i++) {
  if (people[i].name === "Otto") continue;
  combinedAge += people[i].age; // inkludere Otto? fjern "//" på linjen over
}

let averageAge = 0;
averageAge = combinedAge / (people.length - 1); // inkludere Otto? fjern "//" på linje 105 og "-1" på denne linjen
...
function diceRoller(diceNumber, sides) {
  let result = [];
...
let dirtyText = [" thIS", "teXt  ", " nEeds ", "to", "BE", "cleANED   ", " Up"];
function cleanText(array) {
  const cleanedArray = [];
  for (let word of array) {
    cleanedArray.push(word.trim().toLowerCase());
  }
...
// console.log(doubleSwap("hello world", "h", "w"));
...
function helloChecker(string) {
  ...
  const languages = Object.keys(helloWords); // ['english', 'italian', ...]
  const greetings = Object.values(helloWords); // ['hello', 'ciao', ...]
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    for (let j = 0; j < languages.length; j++) {
      ...
      const helloWord = helloWords[language];

      if (word === helloWord) {
        return `HELLO oppdaget på ${language}.`;
      }
    }
  }
...
function helloChecker(string) {
  ...
  for (let word of words) {
    for (let [language, helloWord] of Object.entries(helloWords)) {
      if (word === helloWord) {
        return `HELLO oppdaget på ${language}.`;
      }
    }
